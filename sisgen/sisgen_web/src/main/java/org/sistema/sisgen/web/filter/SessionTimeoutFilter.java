package org.sistema.sisgen.web.filter;
		...
		if ((request instanceof HttpServletRequest)
			...
			if (isSessionControlRequiredForThisResource(httpServletRequest)) {
				...
				if (isSessionInvalid(httpServletRequest)) {
				    ...
				    String timeoutUrl = httpServletRequest.getContextPath() + this.timeoutPage;
					log.debug("La sesion es inválida!, redireccionando a la pagina de login: " + timeoutUrl);
					httpServletResponse.sendRedirect(timeoutUrl);
					
					//RequestDispatcher rd = httpServletRequest.getRequestDispatcher(this.timeoutPage);

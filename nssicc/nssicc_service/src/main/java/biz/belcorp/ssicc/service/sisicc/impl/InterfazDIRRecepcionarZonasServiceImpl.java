...
/**
 * Service para la recepcion de Zonas.
 * 
 * @author <a href="mailto:itocto@sigcomt.com">Ivan Tocto</a>
 */
@Service("sisicc.interfazDIRRecepcionarZonasService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class InterfazDIRRecepcionarZonasServiceImpl extends BaseInterfazEntradaAbstractService {
	...
	protected void processData(InterfazParams interfazParams, List data) throws InterfazException {
		...
		for(int i=0; i< data.size(); i++)
		{
			...
			int valor = interfazDIRDAO.verificarZonaExiste(criteria);
			
			if(valor > 0)
			{
				interfazDIRDAO.updateZona(criteria);
			}
			else
			{
				interfazDIRDAO.insertZona(criteria);
			}			
		}
	}

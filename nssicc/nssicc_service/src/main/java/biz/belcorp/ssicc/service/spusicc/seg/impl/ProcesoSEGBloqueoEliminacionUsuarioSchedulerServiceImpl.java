 ...
 *
 */
public class ProcesoSEGBloqueoEliminacionUsuarioSchedulerServiceImpl extends SchedulerFactoryBean {

	protected final Log log = LogFactory.getLog(getClass());
	
	private GenericoDAO genericoDAO;
	
	private ProcesoSEGBloqueoEliminacionUsuarioService procesoSEGBloqueoEliminacionUsuarioService;
	...
	public void afterPropertiesSet() throws Exception {
		...
		String indicadorEjecucion = obtenerParametroPais(codigoPais, Constants.SISTEMA_SEG, null, Constants.SEG_INDICADOR_PARAMETRO_JOBBLOQUEO);	
		log.debug("indicadorEjecucion " + indicadorEjecucion);
		
		if(Constants.SI.equals(indicadorEjecucion)) {
			if(!super.isRunning()){
	...
	public void setCronExpression() throws ParseException, SchedulerException, Exception {
		log.debug("Entrando .... ProcesoGENReactivacionesGerentesDirectorioSchedulerServiceImpl");
			...
			if(StringUtils.isNotEmpty(periodoEjecucion)){			
                ...
                tarea.setTargetObject(this.procesoSEGBloqueoEliminacionUsuarioService);
                tarea.setTargetMethod("executeProcesoBloqueoEliminacionUsuarioMasiva");
                tarea.setName("TR01_ProcesoSEGBloqueoEliminacionUsuario");
                tarea.setConcurrent(false);
                tarea.afterPropertiesSet();
				
				CronTriggerBean cronTrigger = new CronTriggerBean();
				cronTrigger.setBeanName("CR01_ProcesoSEGBloqueoEliminacionUsuario");
	...
	/**
	 * @return the procesoSEGBloqueoEliminacionUsuarioService
	 */
	public ProcesoSEGBloqueoEliminacionUsuarioService getProcesoSEGBloqueoEliminacionUsuarioService() {
		return procesoSEGBloqueoEliminacionUsuarioService;
	}


	/**
	 * @param procesoSEGBloqueoEliminacionUsuarioService the procesoSEGBloqueoEliminacionUsuarioService to set
	 */
	public void setProcesoSEGBloqueoEliminacionUsuarioService(
			ProcesoSEGBloqueoEliminacionUsuarioService procesoSEGBloqueoEliminacionUsuarioService) {
		this.procesoSEGBloqueoEliminacionUsuarioService = procesoSEGBloqueoEliminacionUsuarioService;
	}

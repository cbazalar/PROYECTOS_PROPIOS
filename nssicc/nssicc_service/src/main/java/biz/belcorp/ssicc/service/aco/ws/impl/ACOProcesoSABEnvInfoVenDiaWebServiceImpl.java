...
import biz.belcorp.ssicc.service.aco.ws.ACOProcesoSABEnvInfoVenDiaWebService;
 ...
 */
public class ACOProcesoSABEnvInfoVenDiaWebServiceImpl extends BaseInterfazHiloAbstractWebService implements ACOProcesoSABEnvInfoVenDiaWebService{
	...
	public ACOWebServiceResponse ejecutarProcesoSABEnvInfoVenDia(
		...
		final String CODIGO_INTERFAZ = "SAB-P2";
		final String CODIGO_SISTEMA = "SAB";
		
		if(log.isDebugEnabled()){
			log.debug("ejecutar Proceso Interfaz ejecutarProcesoSABEnvInfoVenDia");
		}
		try{
			...
			if(StringUtils.isBlank(codigoPeriodo) || StringUtils.isEmpty(codigoPeriodo)){
				codigoPeriodo = this.interfazSiCCService.getPeriodoDefaultByPaisCanal(pais.getCodigo(),Constants.CODIGO_CANAL_DEFAULT); 
			}

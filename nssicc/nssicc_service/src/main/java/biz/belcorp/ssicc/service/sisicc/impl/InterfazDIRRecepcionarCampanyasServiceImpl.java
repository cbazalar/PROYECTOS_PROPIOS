 ...
 */
@Service("sisicc.interfazDIRRecepcionarCampanyasService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class InterfazDIRRecepcionarCampanyasServiceImpl extends BaseInterfazEntradaAbstractService {
	...
	protected void processData(InterfazParams interfazParams, List data) throws InterfazException {
		...
		for(int i=0; i< data.size(); i++)
		{
			...
			int valor = interfazDIRDAO.verificarCampanyaExiste(criteria);
			
			if(valor > 0)
			{
				interfazDIRDAO.updateCampanya(criteria);
			}
			else
			{
				interfazDIRDAO.insertCampanya(criteria);
			}			
		}
	}	
}
 ...
 */
@Service("sto.mailReporteSTOCentroAcopioFacturadoService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class MailReporteSTOCentroAcopioFacturadoServiceImpl extends BaseMailReporteAbstractService {
	/* (non-Javadoc)
	...
	protected String getSubject(MailParams mailParams) throws Exception {
		...
		Map parameterMap = (Map) mailParams.getQueryParams().get("parameterMap");		
		titulo = titulo + " - Centro de acopio: " + StringUtils.trim((String) parameterMap.get("nomCentroAcopio"))
						+ " - Compaia de transporte: " + StringUtils.trim((String) parameterMap.get("ciaTransporte"));
		return titulo;
	}

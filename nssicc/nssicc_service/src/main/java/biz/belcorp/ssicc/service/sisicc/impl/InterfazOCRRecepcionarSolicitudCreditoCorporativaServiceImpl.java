 ...
 * 
 */
@Service("sisicc.interfazOCRRecepcionarSolicitudCreditoCorporativaService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class InterfazOCRRecepcionarSolicitudCreditoCorporativaServiceImpl extends BaseInterfazEntradaAbstractService {
	...
	/* (non-Javadoc)
	 * @see biz.belcorp.ssicc.sisicc.service.framework.BaseInterfazAbstractService#afterProcessInterfazOldVersion(biz.belcorp.ssicc.sisicc.service.framework.beans.InterfazParams)
	 */
	protected void afterProcessInterfazOldVersion(InterfazParams interfazParams)	throws InterfazException {
...
protected void afterProcessInterfaz(InterfazParams interfazParams)	throws InterfazException {
	...
	String indOldVersion = genericoDAO.getParametroPais(codigoPais, "VER", "000");
	
	if (Constants.NUMERO_UNO.equals(indOldVersion)){
		afterProcessInterfazOldVersion(interfazParams);
	}
	else{
		String codTipoDocu = Constants.STO_TIPO_DOCUMENTO_SCC;
		...
		if(Constants.NUMERO_UNO.equals(indActivaWS)){
			...
			if(Constants.PAIS_PE.equals(codigoPais)){
				...
				while(it.hasNext()){
					Map hmap = (Map)it.next();
					String documentoIdentidad = (String)hmap.get("numeroDocumentoIdentidad");
					log.debug("documentoIdentidad "+documentoIdentidad);
					//invocamos ws
					String res=null;
					try{
						
						if(StringUtils.isNotEmpty(documentoIdentidad)){
							
								integrante.setNumeroDocumento(documentoIdentidad);
								integrante.setTipoDocumento(StringUtils.isNotEmpty(tipoDocEqui)?Integer.parseInt(tipoDocEqui):1);//por default 1:dni
								WebServiceEquifax locator = new WebServiceEquifaxLocator();
								log.debug("URL ws Boletin electronico infocorp  >>> "+urlWSOCR);
								WS00230301Endpoint serviceEquifax =locator.getWS00230301EndpointPort(new java.net.URL(urlWSOCR));
								log.warn("Invocacion INFORCORP Inicio : " + documentoIdentidad);
								res=serviceEquifax.consultaServicio16(estructuraServicio16_1);
								log.warn("Invocacion INFORCORP Fin : " + documentoIdentidad);
						}
			...
			if(Constants.PAIS_BOE.equals(codigoPais)){
				
				String tokenWSOCR = genericoDAO.getParametroPais(codigoPais, Constants.SISTEMA_OCR, Constants.OCR_TOKEN_WS_SCCRECEP);
				log.debug("tokenWSOCR Recepcion--------->: " + tokenWSOCR);
				...
				while(it.hasNext()){
					...
					try{
						if(StringUtils.isNotEmpty(documentoIdentidad)){
							...
							serviceSP.setEndpoint(urlWSOCR);
							log.debug("Conecta WS Recepcion--------->: " + serviceSP);
							res = serviceSP.consultar(documentoIdentidad, tokenWSOCR);
							log.debug("WS Invoca al metodo consultar()--------->: (" + documentoIdentidad + ", " + tokenWSOCR + ")");
						}
					}catch(Exception e){
						log.debug("error conexion web service "+ e.getMessage());
						res = null;
						throw new InterfazException("Error en conectarse al servicio web recepcion SCC: " + e.getMessage());
					}	
					if(StringUtils.isNotEmpty(res)){
						//hmap.put("montoInfoCom","");
						//hmap.put("error","");	
						hmap.put("nombresInfocorp","");							
						
						hmap.put("indTeleOk","1"); //Paso por web service
						
						if(res.equals("1")){
								hmap.put("estado", "1");
								hmap.put("indSituCred", res);
								hmap.put("nombresInfocorp", "1: Apto");		
						}
						
					  if(res.equals("0")){
								hmap.put("estado", "12");
								hmap.put("indSituCred", res);
								hmap.put("nombresInfocorp", "0: No Apto");		
						}
						
						if (res.equals("T")) {
							hmap.put("estado", "5");
							hmap.put("indSituCred", "0");
							hmap.put("nombresInfocorp", "T: Token Incorrecto");
						}							
						
						if (res.equals("E")){ 
							hmap.put("estado", "5");
							hmap.put("indSituCred", "0");
							hmap.put("nombresInfocorp", "E: Error de servicio Externo");
						}
						
						if (res.equals("B")) {
							hmap.put("estado", "5");
							hmap.put("indSituCred", "0");
							hmap.put("nombresInfocorp", "B: Error de base de dato");
						}

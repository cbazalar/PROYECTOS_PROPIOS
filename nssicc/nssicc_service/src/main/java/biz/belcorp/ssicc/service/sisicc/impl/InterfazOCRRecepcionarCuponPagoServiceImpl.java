...
/**
 * Service para la recepcin de Cupon de pago en la Interfaz OCR.
 * 
 * @author <a href="mailto:doliva@belcorp.biz">Cristhian Roman</a>
 */
@Service("sisicc.interfazOCRRecepcionarCuponPagoService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class InterfazOCRRecepcionarCuponPagoServiceImpl extends
	...
	protected void beforeReadData(InterfazParams interfazParams)
		...
		try {
			...
			interfazOCRDAO.deleteInterfazOCRRecepcionarCuponPago();	
			interfazOCRDAO.executeInterfazOCRRecepcionarCuponPago(map);			
			
			
		} catch (Exception e) {
			
			throw new InterfazException(
						"Error al borrar/cargar los registros de la tabla : "+ e.getMessage());
	...
	protected void afterProcessInterfaz(InterfazParams interfazParams)	throws InterfazException {
		...
		String codTipoDocu = Constants.STO_TIPO_DOCUMENTO_CUP;
		...
		try {
			interfazOCRDAO.executeInterfazOCRProcesarConsolidadoCuponPago(map);
		} catch (Exception e) {
			throw new InterfazException("Error al procesar executeInterfazOCRProcesarConsolidadoCuponPago: "	+ e.getMessage());
		}

...
@Service("spusicc.procesoLECCalcularRecuperacionService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class ProcesoLECCalcularRecuperacionServiceImpl extends BaseInterfazProcesoAbstractService{
		...
		if(tipoProceso == null){
		  ...
		  if(StringUtils.equals(Constants.CODIGO_TIPO_CIERRE_REGION, tipoProceso)){
			for(int i=0; i<codigoRegiones.length; i++) {
				...
				procesoLECCalcularRecuperacionDAO.executeProcesoLECCalcularRecuperacion(params);
			}
		  }else{

...
@Service("sisicc.interfazXRXRecepcionarBoletaVentaElectronicaService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class InterfazXRXRecepcionarBoletaVentaElectronicaServiceImpl extends
	...
	protected void processData(InterfazParams interfazParams, List data)
		...
		try {
			...
			while(it.hasNext()) {
				...
				try {
					String codigoCorrelativo = (String)row.get("codigoCorrelativo");
					String folio = (String)row.get("folio");
					String codigoBarra = (String)row.get("codigoBarra");
					boolean error = false;
					
					if(StringUtils.isBlank(folio))
					{
						error = true;
						mensajeError = messageSource.getMessage("interfazXRXBoletaVentaElectronicaForm.error.noexiste.folio",new String[]{codigoCorrelativo},getLocale(usuario));						
					}
					
					if(error)
						throw new InterfazException(mensajeError);
					
					interfazXRXDAO.updateInterfazXRXRecepcionarBoletaVentaElectronica(row);
			...
			cantidadRegistros = interfazXRXDAO.getCantidadBoletasVenta(nombreArchivoSinExtension);
			...
			if( registrosErroneos > 0){
				interfazParams.appendLog(messageSource.getMessage("interfazXRXBoletaVentaElectronicaForm.error.folio.nulo",new String[]{},getLocale(usuario)));
				throw new InterfazException(messageSource.getMessage("interfazXRXBoletaVentaElectronicaForm.error.folio.nulo",new String[]{},getLocale(usuario)));
			}
		}		
	...
	protected void afterProcessInterfaz(InterfazParams interfazParams)
		...
		if( registrosErroneos == 0){
			...
			log.debug(sp2[0]);
			interfazXRXDAO.executeBoletaVentaAHistorico(params);
		}
	}
}

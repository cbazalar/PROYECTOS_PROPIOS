...
/**
 * Service para la recepcion de Regiones.
 * 
 * @author <a href="mailto:itocto@sigcomt.com">Ivan Tocto</a>
 */
@Service("sisicc.interfazDIRRecepcionarRegionesService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class InterfazDIRRecepcionarRegionesServiceImpl extends BaseInterfazEntradaAbstractService {
	...
	protected void processData(InterfazParams interfazParams, List data) throws InterfazException {
		...
		for(int i=0; i< data.size(); i++)
		{
			...
			int valor = interfazDIRDAO.verificarRegionExiste(criteria);
			
			if(valor > 0)
			{
				interfazDIRDAO.updateRegion(criteria);
			}
			else
			{
				interfazDIRDAO.insertRegion(criteria);
			}			
		}
	}

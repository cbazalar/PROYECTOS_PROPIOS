 ...
 * 
 * <p>
 * <a href="InterfazSGRRecepcionarPolizasCanceladasServiceImpl.java.html"> <i>View Source</i> </a>
 * </p>
 * 
 * @author <a href="mailto:sbuchelli@belcorp.biz">Sergio Buchelli</a>
 * 
 */
@Service("sisicc.interfazSGRRecepcionarPolizasCanceladasService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class InterfazSGRRecepcionarPolizasCanceladasServiceImpl extends BaseInterfazEntradaAbstractService {
		...
		try {
			...
			while(it.hasNext()) {
				...
				/* Insertando en la cabecera */ 
					try {	
						String codigoConsultora = (String)row.get("codigoCliente");
						String campanha = (String)row.get("campanhaCancelacion");
						String numeroPoliza = (String)row.get("numeroPoliza");
						interfazSGRDAO.executeInterfazSGRRecepcionarPolizasCanceladas(row);
						
						int codigoError = Integer.parseInt((String)row.get("codigoError"));
						mensajeError = (String)row.get("mensajeError");
						 
						switch (codigoError) {
						case 0://no existe tipo logro
							mensajeError = messageSource.getMessage("interfazSGRRecepcionarPolizasCanceladasForm.error.noexiste.poliza",new String[]{numeroPoliza},getLocale(usuario));
							throw new InterfazException(mensajeError);
						case 1:	//no existe consultora en poliza					
							mensajeError = messageSource.getMessage("interfazSGRRecepcionarPolizasCanceladasForm.error.noexiste.consultora",new String[]{codigoConsultora},getLocale(usuario));
							throw new InterfazException(mensajeError);							

						case 2://no existe consulora en el maestro de clientes							
							mensajeError = messageSource.getMessage("interfazSGRRecepcionarPolizasCanceladasForm.error.noexiste.consultora.maestro",new String[]{codigoConsultora},getLocale(usuario));
							throw new InterfazException(mensajeError);							
							
						case 3://no existe campanha							
							mensajeError = messageSource.getMessage("interfazSGRRecepcionarPolizasCanceladasForm.error.noexiste.campanha",new String[]{campanha},getLocale(usuario));
							throw new InterfazException(mensajeError);							
						}
						...
						if(ex.getCause()!=null) {
							...
							if(tex.getCause()!=null)
								...
								log.debug("Registro Nro: " + (i + 1) + ". " +  tex.getCause().getMessage());
							else
								//interfazParams.appendLog("Registro Nro: " + (i + 1) + ". " +  tex.getMessage());
								log.debug("Registro Nro: " + (i + 1) + ". " +  tex.getMessage());
						} else {
							log.debug("Registro Nro: " + (i + 1) + ". " +  ex.getMessage());

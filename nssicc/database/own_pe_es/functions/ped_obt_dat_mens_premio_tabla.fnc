CREATE OR REPLACE FUNCTION "PED_OBT_DAT_MENS_PREMIO_TABLA" (
   ...
   IDCANAL IN NUMBER,
   IDIDIOMA IN NUMBER)
RETURN OBJ_DAT_MEN_PRE_TABLE PIPELINED IS
f_registro OBJ_PED_OBT_DAT_MENSAJE_PREMIO := OBJ_PED_OBT_DAT_MENSAJE_PREMIO(null,null,null,null,null,null,
 ...
 F_VAL_INFO_EBEL_NOTI NUMBER(1);
  ...
  FROM ped_solic_cabec,
       ...
       WHERE ped_solic_cabec.oid_soli_cabe = IDSOLICCABECERA
         ...
         AND rec_linea_opera_recla.prod_oid_prod IN (SELECT PROD_OID_PROD FROM PED_SOLIC_POSIC WHERE SOCA_OID_SOLI_CABE = IDSOLICCABECERA AND VAL_CODI_VENT IS NOT NULL)
     ...
     OPEN f_cur_ped_solic_cabec;
       LOOP
          fetch f_cur_ped_solic_cabec
           ...
           F_PROD_OID_PROD, F_VAL_INFO_EBEL_NOTI;

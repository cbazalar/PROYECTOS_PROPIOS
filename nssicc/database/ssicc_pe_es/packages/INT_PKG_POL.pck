create or replace package INT_PKG_POL is
...
END INT_PKG_POL;
/
create or replace package body INT_PKG_POL is
...
PROCEDURE INT_PR_POL_ENVIO_CONSU(psCodigoPais VARCHAR2,
   ...
   CURSOR c_interfaz(vnIdPais NUMBER, vnIdMarca NUMBER, vnIdCanal NUMBER, vsCodPeriAnterior VARCHAR2, vnIdPeriAnterior NUMBER,fechaActualizacionInterfaz DATE) IS
     ...
     select cli.cod_clie,
            ...
            (
             ...
             CASE substr(ECL.COD_ESTA_CLIE,2,1) 
               when '1' then '0' 
               when '2' then '1'
               when '3' then '2' 
               when '5' then '4'
    ...
    /*         
        (SELECT DECODE(COUNT(1),0,'N','S')  
         ...
         WHERE clbl.tibq_oid_tipo_bloq = tibq.oid_tipo_bloq
           ...
           AND clbl.clie_oid_clie =cli.oid_clie)*/
        FROM mae_clien cli,
             mae_clien_direc cdi,
             mae_clien_ident mi,
   ...
   BEGIN
       ...
       SELECT MAX(FEC_IPRO) INTO pdFechaUltimaActualizacion
        ...
        OPEN c_interfaz(lnIdPais,lnIdMarca,lnIdCanal, lsCodPeriAnterior, lnIdPeriAnterior ,pdFechaUltimaActualizacion)  ;
        LOOP
...
CURSOR c_interfaz IS    
    ...
    select cp.val_nomb_peri,
         ...
         --  psp.val_codi_vent,
           mp.cod_sap,
           gen.val_i18n,
           (select sum(p.num_unid_aten)
              from ped_solic_posic p
             where p.soca_oid_soli_cabe = psc.oid_soli_cabe)
...
IS  
 CURSOR c_interfaz(vnIdPeriodo NUMBER) IS    
   ...
   TYPE interfazRec IS RECORD(
        ...
        tipoCargo             ccc_subpr.des_subp%TYPE,
...
IS                                   
 ...
 CURSOR c_interfaz(vnIdPeriodo NUMBER) IS    
   ...
   TYPE interfazRec IS RECORD(
        ...
        tipoAbono             ccc_subpr.des_subp%TYPE,

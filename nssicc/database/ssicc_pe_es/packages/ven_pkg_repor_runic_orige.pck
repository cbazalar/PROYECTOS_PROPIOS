CREATE OR REPLACE PACKAGE "VEN_PKG_REPOR_RUNIC_ORIGE" IS
...
END VEN_PKG_REPOR_RUNIC_ORIGE;
/

CREATE OR REPLACE PACKAGE BODY "VEN_PKG_REPOR_RUNIC_ORIGE"
IS
...
IS
  ...
  IS
      SELECT
           ...
           '' AS COD_CLIE,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_DEVUE_HOMO_SUNAT(F.TIDO_OID_TIPO_DOCU,F.PAIS_OID_PAIS) AS DOC_LEG,
           F.VAL_SERI_DOCU_LEGA,
           VEN_FN_DEVUE_COMPR_PAGO(F.VAL_NUME_DOCU_LEGA,8) AS VAL_NUME_DOCU_LEGA,
           F.VAL_NUME_IDEN_FISC,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_DEVUE_HOMO_SUNAT(F.TIDO_TIPO_DOCU_REFE,F.PAIS_OID_PAIS) AS DOC_REF,
           F.VAL_SERI_DOCU_REFE,
           F.VAL_NUME_DOCU_REFE,
           '' AS DOC_INI,
           '' AS DOC_FINA,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_BASE_IMPOR(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO ) AS VAL_BASE_IMPO,
           DECODE (F.TAIM_OID_TASA_IMPU,2002,0,ABS(F.VAL_DESC)) VAL_DESC,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_IMPOR_NETO(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO_NETO) AS VAL_BASE_IMPO_NETO,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_OPERA_INAFE(F.TAIM_OID_TASA_IMPU, abs(F.VAL_BASE_IMPO)) as OPINAF,
           ...
           '' AS DOC_INI_AUX,
           '' AS DOC_FINA_AUX,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_BASE_IMPOR(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO ) AS VAL_BASE_IMPO_AUX,
           DECODE (F.TAIM_OID_TASA_IMPU,2002,0, ABS(F.VAL_DESC)) VAL_DESC_AUX,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_IMPOR_NETO(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO_NETO) AS VAL_BASE_IMPO_NETO_AUX,
           VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_OPERA_INAFE(F.TAIM_OID_TASA_IMPU, abs(F.VAL_BASE_IMPO)) as OPINAF_AUX,
  ...
  IS
        SELECT
           ...
           SUM(VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_BASE_IMPOR(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO )) AS VAL_BASE_IMPO,
           SUM(ABS(F.VAL_DESC)) AS VAL_DESC,
           SUM(VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_IMPOR_NETO(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO_NETO)) AS VAL_BASE_IMPO_NETO,
           SUM(VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_OPERA_INAFE(F.TAIM_OID_TASA_IMPU, abs(F.VAL_BASE_IMPO))) as OPINAF,
       ...
       GROUP BY F.FEC_EMIS, F.TIDO_OID_TIPO_DOCU, F.VAL_SERI_DOCU_LEGA, VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_DEVUE_HOMO_SUNAT(F.TIDO_OID_TIPO_DOCU,F.PAIS_OID_PAIS);
           ...
           IF tablaRegistro.COUNT > 0 THEN
             FOR x IN tablaRegistro.FIRST .. tablaRegistro.LAST LOOP
                 ...
                 /*BEGIN
                     ...
                     INTO REG.BOL_DESP
                     FROM FAC_REGIS_UNICO_VENTA F,
                          fac_docum_conta_cabec_origen D,
                          ped_solic_cabec_origen S
                 ...
                 IF REG.TIDO_OID_TIPO_DOCU = '07' THEN
                    ...
                    /*IF lsCodSbac = '000' THEN
                       BEGIN
                          ...
                          FROM fac_docum_conta_cabec_origen A,
                               ped_solic_cabec_origen B,
                               ped_solic_cabec_origen C
...
IS
  ...
  IS
       ...
       SELECT
           ...
           F.VAL_BASE_IMPO,
           F.VAL_DESC,
           F.VAL_BASE_IMPO_NETO,
           DECODE (F.IMP_IMPU,0,F.IMP_TOTA,0) AS OPINAF ,
           F.IMP_IMPU,
           F.IMP_TOTA,
           F.IND_ESTA,
           F.IND_TRAN_GRAT,
...
IS
  lnIdPais         SEG_PAIS.OID_PAIS%TYPE;
BEGIN
   DELETE FROM  VEN_REPOR_REGIS_VENTA_CONSO;
  ...
  SELECT
       ...
       VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_DEVUE_DOCUM_LEGAL(F.TIDO_OID_TIPO_DOCU,F.PAIS_OID_PAIS,F.VAL_NUME_DOCU_LEGA) AS DOC_LEG,
       F.VAL_SERI_DOCU_LEGA,
       F.IND_ESTA,
       F.IND_TRAN_GRAT,
       VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_BASE_IMPOR(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO ),
       ABS(F.VAL_DESC),
       VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_IMPOR_NETO(F.TAIM_OID_TASA_IMPU, F.VAL_BASE_IMPO_NETO),
       ABS(F.IMP_IMPU),
       ABS(F.IMP_TOTA),
       VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_DEVUE_HOMO_SUNAT(F.TIDO_OID_TIPO_DOCU, lnIdPais) as COD_TIPO_DOCU,
       VEN_PKG_REPOR_RUNIC_ORIGE.VEN_FN_OBTIE_OPERA_INAFE(F.TAIM_OID_TASA_IMPU, abs(F.VAL_BASE_IMPO)),
       IND_ESTA
FROM  FAC_REGIS_UNICO_VENTA F,
      SEG_SUBAC S,

CREATE OR REPLACE PACKAGE GEN_PKG_FTP AS
-- --------------------------------------------------------------------------
...
PROCEDURE rmdir (p_conn  IN OUT NOCOPY  UTL_TCP.connection,
   ...
   PROCEDURE ftp_pr_envio_ascci(
      ...
      p_dir_dest                        IN         VARCHAR2,
      p_nom_arch_dest             IN         VARCHAR2);

   PROCEDURE ftp_pr_delet_ascii(
      p_host IN VARCHAR2,
      p_port IN VARCHAR2,
      p_user IN VARCHAR2,
      p_pass IN VARCHAR2,
...
CREATE OR REPLACE PACKAGE BODY GEN_PKG_FTP AS
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
   ...
   BEGIN
      ...
      EXECUTE IMMEDIATE 'CREATE OR REPLACE DIRECTORY SSICC_DIR_ENVI_FTP AS ''' || p_dir || '''';

      l_conn := login(p_host,p_port,p_user,p_pass);
      ascii(l_conn);
      put(l_conn,'SSICC_DIR_ENVI_FTP',p_nom_arch,p_dir_dest || p_nom_arch_dest );
      logout(l_conn);

   END ftp_pr_envio_ascci;
   ...
   BEGIN
      ...
      l_conn := login(p_host,p_port,p_user,p_pass);
      ascii(l_conn);
      delete_file(l_conn,p_dir || p_nom_arch);
      logout(l_conn);

   END ftp_pr_delet_ascii;

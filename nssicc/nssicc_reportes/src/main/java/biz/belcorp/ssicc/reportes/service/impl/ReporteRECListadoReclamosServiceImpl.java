...
@Service("reportes.reporteRECListadoReclamosService")
@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
public class ReporteRECListadoReclamosServiceImpl extends BaseReporteInterfaceImpl{
	...
	public ReporteParams beforeExecuteReporte(ReporteParams reporteParams, BaseReporteForm formReporte) throws Exception {
		if (log.isDebugEnabled()) {
			log.debug("Entering 'ReporteRECListadoReclamosServiceImpl.beforeExecuteReporte' method");
		}
		...
		if (StringUtils.equals(Constants.IND_MOVIMIENTO_DEVUELVE,
			...
			reporteService.insertTableReportesSQL(
					"insertReporteRECListadoReclamosDevuelve", params);
		else
			reporteService.insertTableReportesSQL(
					"insertReporteRECListadoReclamosEnvia", params);

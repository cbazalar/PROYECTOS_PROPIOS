...
/*	@ManagedProperty(value="#{mantenimientoPEDFleteSearchAction}")
	...
	*/
	...
	protected void setViewAtributes() throws Exception {
		...
		if (mPadre!=null) {
			if (!mPadre.getAccion().equals(this.ACCION_NUEVO)) {
				
				if (mPadre.getAccion().equals(this.ACCION_CONSULTAR)) {
					f.setEditable(false);
				}
				...
				//Obtenemos el ID 
				this.oidFlete = MapUtils.getString(mapa, "oidFlete");
				if(StringUtils.isNotBlank(this.oidFlete) && Integer.parseInt(this.oidFlete) > 0){
						listaFleteDetalle = servicePed.getDetalleFleteList(this.oidFlete);
	...
	public void insertarRegistro(ActionEvent actionEvent) {
	   ...
	   MantenimientoPEDFleteDetalleForm detForm = (MantenimientoPEDFleteDetalleForm)this.formBusqueda;
		...
		float rangoInferior = Float.parseFloat(detForm.getRangoInferior());
		float rangoSuperior= Float.parseFloat(detForm.getRangoSuperior());
		
		if(rangoInferior>rangoSuperior){			
			this.addWarn("", this.getResourceMessage("mantenimientoPEDFleteDetalleForm.rangos.validation"));
			return;
		...
		bean.setOidFlete(this.oidFlete);

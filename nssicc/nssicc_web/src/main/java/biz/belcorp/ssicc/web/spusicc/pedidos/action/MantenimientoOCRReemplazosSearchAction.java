...
public class MantenimientoOCRReemplazosSearchAction extends
	...
	public void activar(ActionEvent actionEvent) {
		...
		try {
			...
			this.addInfo("Info: ", this.getResourceMessage("registro.ok.activado"));			
		} catch (Exception e) {			
	...
	/**
	 * Desactiva registro en la grilla
	 * 
	 * @param actionEvent
	 */
	public void desactivar(ActionEvent actionEvent) {
		...
		try {
			...
			this.addInfo("Info: ", this.getResourceMessage("registro.error.desactivado"));			
		} catch (Exception e) {
			this.addError("Error: ", this.obtieneMensajeErrorException(e));
		}
	...
	public void loadcodigoVentaPrincipal(AjaxBehaviorEvent e) {
		...
		if (pro == null) {
			...
			if (!pro.getIndicadorExisteReemplazo().equals("0")) {
	...
	public void loadcodigoVentaReemplazo(AjaxBehaviorEvent e) {
		...
		if (pro == null) {
			f.setCodigoVentaReemplazo("");
			f.setCodigoSAPReemplazo("");
			f.setDescripcionReemplazo("");
			this.addError("Error",
					this.getResourceMessage("mensaje.CUV.noExiste"));
		} else {
			if (!StringUtils.isBlank(pro.getCodigoProducto()))
	...
	public void loadSubTiposClientes(ValueChangeEvent val) {
		...
		if (valor != null) {
			...
			this.ocrSubtipoiClienteList = ajaxService
					.getSubTiposClientesPorPaisTipoClientesOID(pais.getCodigoIdiomaIso(),
							myList);
		} else {

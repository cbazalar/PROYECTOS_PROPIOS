...
@ManagedBean
@SessionScoped
public class MantenimientoOCRCapturaPedidosAction1 extends BaseMantenimientoSearchAbstractAction {
	...
	private List<CabeceraDetalleTO> listaPedido;
	private CabeceraDetalleTO cabeceraDetalleTO;
	private List<CabeceraDetalleTO> seleccionados;
	private CabeceraDetalleTO seleccionado;
	private CabeceraDetalleTO[] seleccionadosDP;
	private PedidoDataModel pedidoDataModel;
	
	private PedidoDataModel pedidoDataModelTest;
	private List<CabeceraDetalleTO> listaPedidoTest;
	...
	private boolean mostrarPanelPedido = false;
...
//				busquedaForm.setNumeroDocIdentidadBuscar(cliente.);				
...
//        f.setNumLote(serviceFact.getNumLotes(map));
        ...
        */
        ...
        this.listaPedido = new ArrayList<CabeceraDetalleTO>();
        
        this.pedidoDataModel = new PedidoDataModel(this.listaPedido);
        this.valorFocus = "1";
        this.mostrarCabeceraFija = true;
//        this.mostrarBotonEjecutarProl = true;
	...
	public void addRow(){
		...
		this.listaPedido.add(new CabeceraDetalleTO());
		this.pedidoDataModel = new PedidoDataModel(this.getListaPedido());
		this.valorFocus = "2";
	}
	
	/**
	 * Metodo que realiza el seguimiento del valor de la unidad de demanda ingresado por pantalla de digitacion de pedidos.
	 */
	
	public void ingresaUnidadDemanda(String rowIndex){
		if(log.isDebugEnabled()){
			log.debug("ingresaUnidadDemanda");
		}
		
		this.cabeceraDetalleTO = this.listaPedido.get(new Integer(rowIndex).intValue());
		FacesContext context = FacesContext.getCurrentInstance();
		DataTable dataTable = (DataTable) context.getViewRoot().findComponent(":principalForm:idDataTableS");
		int valor = dataTable.getRowIndex();
		log.debug(valor);		
		if(StringUtils.isNotBlank(this.getCabeceraDetalleTO().getUnidDemanda())
			&& StringUtils.isNotEmpty(this.getCabeceraDetalleTO().getUnidDemanda())){
			this.getCabeceraDetalleTO().setUnidDemanda(this.getCabeceraDetalleTO().getUnidDemanda());
			Double preTotalFormat = Double.valueOf(this.getCabeceraDetalleTO().getValorPrecCatalogo()) * Double.valueOf(this.getCabeceraDetalleTO().getUnidDemanda());
			String valorTransformado = NumberUtil.NumberToString(NumberUtils.toDouble(preTotalFormat.toString(),0.00d));
			this.getCabeceraDetalleTO().setPreTotalFormat(valorTransformado);
			//((CabeceraDetalleTO)this.getListaPedido().get(Integer.parseInt(rowIndex))).setPreTotalFormat(String.valueOf(preTotalFormat));
			this.getListaPedido().set(Integer.parseInt(rowIndex), this.getCabeceraDetalleTO());
			this.addRow();
		}
	}
	 ...
	 */
	public void ingresoCodigoVenta(String rowIndex){	
		if (log.isDebugEnabled()) {
		...
		if(existeCodCons){
			...
			if(StringUtils.isNotBlank(this.getCabeceraDetalleTO().getCodigoVenta()) 
				...
				this.getCabeceraDetalleTO().setCodigoVenta(this.getCabeceraDetalleTO().getCodigoVenta());
				if(existeCodigoInListaDetPed(this.getCabeceraDetalleTO().getCodigoVenta())>0){
					...
					obtenerObjeto("codigoVenta",idCodigoVenta,listado);
					this.setMensajeLocal(this.getResourceMessage("mensaje.codigoVtaExiste"));
					this.posicionarUniDem();
					this.index = -1;*/
				}else{			
					this.index = validaExisteCodigoVenta(this.getCabeceraDetalleTO().getCodigoVenta());
					if(this.getIndex()!=-1){
						int existeCodVenta = validaCodigoVentaInListaCabecera(this.getCabeceraDetalleTO().getCodigoVenta());
						if(existeCodVenta==1){
						...
						if(existeCodVenta > 1){
							((CabeceraDetalleTO)this.listaPedido.get(Integer.parseInt(rowIndex))).setCodigoVenta("");
	 ...
	 * @return the int
	 */
	public int validaExisteCodigoVenta(String idCodigoVenta){
		int index = -1;
		for (int i = 0; i < listaCodigosMatriz.size(); i++) {
			if(((LabelPedidosValue)listaCodigosMatriz.get(i)).getCodigoVta().equals(idCodigoVenta)){
				index = i;
				break;
			}			
		}		
	...
	public boolean existenCUVsInListaDetPed(){
		...
		for (CabeceraDetalleTO elemento : listaPedido) {
			if(CollectionUtils.exists(
					listaDetallePedido, 
					new BeanPredicate("codigoVenta", 
							new EqualPredicate(elemento.getCodigoVenta())))){
				return true;
			}
		}
		return false;
...
//				}
	 ...
	 * @return the int
	 */
	public int validaCodigoVentaInListaCabecera(String idCodigoVenta){
		if(log.isDebugEnabled()){
			log.debug("validaCodigoVentaInLista");
		}
		int i = 0;
		BeanPredicate beanPredicate = new BeanPredicate("codigoVenta", new EqualPredicate(idCodigoVenta));
		i = CollectionUtils.countMatches(this.getListaPedido(), beanPredicate);	
		return i;	
	}
	...
	public void limpieza(){
		...
		this.pedidoDataModel = new PedidoDataModel(new ArrayList<CabeceraDetalleTO>(this.getListaPedido()));
	}	
		...
		if(StringUtils.isNotBlank(busquedaForm.getCodigoConsultora())
			...
			if(existeConsultora()){
				 ...
				 if(existeBloqueoOnline()>0){
					...
					if(!lista.isEmpty()){
			    		if(lista.size()>0){		    			
			    			this.listaPedido = conversionLista(lista);
			    			this.pedidoDataModel = new PedidoDataModel(this.getListaPedido());
			    		...
			    		this.setListaPedido(new ArrayList<CabeceraDetalleTO>());
			    		this.getListaPedido().add(new CabeceraDetalleTO());
			    		this.pedidoDataModel = new PedidoDataModel(this.getListaPedido());
			    		this.mostrarPanelPedido = true;
			    	}
		        }
			}else{
	 ...
	 * @param e the e
	 */
	public void mostrarDialogoDelete(){
...
//		String[]objlistaUnidades  = f.getLabel4();
		String[]objlistaCodigos   = null;
		String[]objlistaUnidades  = null;
...
//			saveErrors(session, errors);
	...
	public List<CabeceraDetalleTO> getListaPedido() {
		return listaPedido;
	}

	public void setListaPedido(List<CabeceraDetalleTO> listaPedido) {
		this.listaPedido = listaPedido;
	}

	public boolean isMostrarPanelPedido() {
		return mostrarPanelPedido;
	}

	public void setMostrarPanelPedido(boolean mostrarPanelPedido) {
		this.mostrarPanelPedido = mostrarPanelPedido;
	}

	public PedidoDataModel getPedidoDataModelTest() {
		return pedidoDataModelTest;
	}

	public void setPedidoDataModelTest(PedidoDataModel pedidoDataModelTest) {
		this.pedidoDataModelTest = pedidoDataModelTest;
	}

	public List<CabeceraDetalleTO> getListaPedidoTest() {
		return listaPedidoTest;
	}

	public void setListaPedidoTest(List<CabeceraDetalleTO> listaPedidoTest) {
		this.listaPedidoTest = listaPedidoTest;
	}

	public CabeceraDetalleTO getSeleccionado() {
		return seleccionado;
	}

	public void setSeleccionado(CabeceraDetalleTO seleccionado) {
		this.seleccionado = seleccionado;
	}

	public List<CabeceraDetalleTO> getSeleccionados() {
		return seleccionados;
	}

	public void setSeleccionados(List<CabeceraDetalleTO> seleccionados) {

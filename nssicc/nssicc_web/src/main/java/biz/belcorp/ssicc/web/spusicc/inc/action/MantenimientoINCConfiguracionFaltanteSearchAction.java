...
public class MantenimientoINCConfiguracionFaltanteSearchAction extends BaseMantenimientoSearchAbstractAction
{
	...
	{		
		...
		if(f.getOidConcurso().length > 0 && !f.getOidConcurso()[0].equals(""))
			criteria.put("oidConcurso", f.getOidConcurso());
		
		if(f.getOidRegion().length > 0 && !f.getOidRegion()[0].equals(""))
			criteria.put("oidRegion", f.getOidRegion());
		
		if(f.getOidZona().length > 0 && !f.getOidZona()[0].equals("")) {
		...
		if(f.getOidPremio().length > 0 && !f.getOidPremio()[0].equals("")) {

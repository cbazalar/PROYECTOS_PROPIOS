...
public class MantenimientoPEDMontoMaximoSearchAction extends BaseMantenimientoSearchAbstractAction
{
	...
	protected List setFindAttributes() throws Exception 
	{
		...
		for(int i = 0; i<lista.size(); i++){
			Map aux = (Map)lista.get(i);
			aux.put("indicadorSeleccionado", false);
		}
		...
		this.datatableBusqueda = new DataTableModel(lista);
...
//		request.getSession(true).removeAttribute(Constants.PED_MONTO_MAXIMO_LIST);
	...
	public void modificarIndMontoMaximo(ActionEvent event) 
	{
		...
		try {
			Map registroSeleccionado = this.devuelveSeleccionado();
			...
			if (registroSeleccionado == null){
				if(this.cont == 0){
					...
					if(this.cont > 1)
						this.addWarn("", this.getResourceMessage("errors.select.unique.item"));
				}
			}else 
			{
				MantenimientoPEDMontoMaximoService service = (MantenimientoPEDMontoMaximoService) getBean("spusicc.mantenimientoPEDMontoMaximoService");
				...
				CarParamCarte carParamCarte = new CarParamCarte();
				if (oidParaCart != null && niriOidNiveRies != null
						&& valI18n != null && valMontMaxiPerm != null
						&& indMontoMaximo != null) 
				{
					carParamCarte.setOidParaCart(oidParaCart);
					carParamCarte.setNiriOidNiveRies(niriOidNiveRies);
			...
			if (registroSeleccionado == null){
				...
				if (StringUtils.isNotBlank(oidParaCart)) {
					...
					if (!listCarParamCarte.isEmpty()) {
						CarParamCarte paramCarte = (CarParamCarte) listCarParamCarte.get(0);
					...
					if (listRegiones != null) {
						...
						for (int i = 0; i < listRegiones.size(); i++) {
							regiones[i] = MapUtils.getString(
									(Map) listRegiones.get(i), "codigoRegion");
						}
					...
					if (listZonas != null) {
						...
						for (int i = 0; i < listZonas.size(); i++) {
							zonas[i] = MapUtils.getString(
									(Map) listZonas.get(i), "codigoZona");
						}
					...
					if (listSecciones != null) {
						...
						for (int i = 0; i < listSecciones.size(); i++) {
							secciones[i] = MapUtils
									.getString((Map) listSecciones.get(i),
											"codigoSeccion");
						}
	...
	public void guardarUA(ActionEvent event) {
		...
		try {
			 ...
			 * Map criteriaCount = new HashMap();
			 * criteriaCount.put("codigoPais", f.getCodigoPais());
			 * criteriaCount.put("codigoNivelRiesgo",
			 * Integer.parseInt(f.getCodigoNivelRiesgo()));
			 * criteriaCount.put("listRegion", (f.getCodigoRegion() == null) ?
			 * new ArrayList() : Arrays.asList(f.getCodigoRegion()));
			 * criteriaCount.put("listZona", (f.getCodigoZona() == null) ? new
			 * ArrayList() : Arrays.asList(f.getCodigoZona()));
			 * criteriaCount.put("listSeccion", (f.getCodigoSeccion() == null) ?
			 * new ArrayList() : Arrays.asList(f.getCodigoSeccion()));
			 */

			Map criteria = new HashMap();
			criteria.put("codigoNivelRiesgo",
					Integer.parseInt(f.getCodigoNivelRiesgo()));
			criteria.put(
					"listRegion",
					(f.getCodigoRegion() == null) ? new ArrayList() : Arrays
							.asList(f.getCodigoRegion()));
			criteria.put(
					"listZona",
					(f.getCodigoZona() == null) ? new ArrayList() : Arrays
							.asList(f.getCodigoZona()));
			criteria.put(
					"listSeccion",
					(f.getCodigoSeccion() == null) ? new ArrayList() : Arrays
							.asList(f.getCodigoSeccion()));
			criteria.put("codigoPais", f.getCodigoPais());
			criteria.put("montoMaximo", f.getMontoMaximo());
			criteria.put("usuario", usuario.getLogin());
	...
	public void loadSeccion(ValueChangeEvent event)
	{
		...
		if(zonasValue.size() > 0 && regionesValue.size() > 0)
	...
	public Map devuelveSeleccionado(){
		List lista = this.listaBusqueda;
		Map registroSeleccionado = null;
		this.cont = 0;
		for(int i=0;i<lista.size();i++){
			Map aux = (Map)lista.get(i);
			if(MapUtils.getBoolean(aux, "indicadorSeleccionado")){
				this.cont++;
			}
		}
	...
	/**
	 * metodo que elimina en registro seleccionado
	 * @param event
	 */
		...
		try {
			...
			if (registroSeleccionado == null){
				...
				if(oidParaCart != null && niriOidNiveRies != null && valI18n != null && valMontMaxiPerm != null && indMontoMaximo != null)
				{
	        		...
	        		this.addInfo("", this.getResourceMessage("mantenimientoPEDMontoMaximoSearchForm.cabecera.deleted"));

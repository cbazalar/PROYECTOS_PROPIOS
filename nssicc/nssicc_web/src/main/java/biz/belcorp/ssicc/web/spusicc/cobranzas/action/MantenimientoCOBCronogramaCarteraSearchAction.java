...
public class MantenimientoCOBCronogramaCarteraSearchAction extends
	...
	protected boolean setSaveAttributes() throws Exception {
		...
		if (accion.equals(this.ACCION_MODIFICAR)) {
			...
			// INICIO PER-SiCC-2015-0168
			if (tipoExcepcion.equals("CIE")) {
				existeCateraAsignada = service.existeCarteraAsignada(params);

				if (existeCateraAsignada == 1) {
					service.cierreCartera(params);
				}
			}
			// FIN PER-SiCC-2015-0168
		}
	...
	public void loadEtapas(ValueChangeEvent val) {
		...
		try {
			...
			if (StringUtils.isBlank(codSociedad)) return;
			AjaxService ajax = (AjaxService) getBean("ajaxService");
			Pais pais = this.mPantallaPrincipalBean.getCurrentCountry();
			this.siccEtapaDeudaList = ajax.getEtapasDeudaByPaisSociedad(
					pais.getCodigo(), codSociedad);
		...
		try {
			...
			if (!StringUtils.isBlank(valor)) {
				...
				this.siccZonaList = ajax
						.getZonasByPaisSociedadEtapaDeudaRegion(
								this.mPantallaPrincipalBean.getCurrentCountry()
										.getCodigo(), f.getCodigoSociedad(), f
										.getCodigoEtapaDeuda(), valor);
			}
		} catch (Exception e) {
			this.addError("Error : ", this.obtieneMensajeErrorException(e));
		}

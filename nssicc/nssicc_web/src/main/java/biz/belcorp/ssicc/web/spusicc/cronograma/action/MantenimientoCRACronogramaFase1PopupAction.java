...
@SessionScoped
@ManagedBean
public class MantenimientoCRACronogramaFase1PopupAction extends
			...
			if (!matriGrupoZona.get("grupoZona").toString().equals("xxx")) {
				...
				if ((orden >= 0) && (orden < 21)) {
					...
					String[] actividad = StringUtils.split(
							matriGrupoZona.get("actividad").toString(), "|");
					...
					for (int j = 0; j < actividad.length; j++) {
						...
						matriFuera.put("orden", Integer.parseInt(matriGrupoZona
								.get("orden").toString()) + 1);
				...
				oidGrupoZonaAnt = matriGrupoZona.get("oidGrupoZona").toString();
			}
	...
	public void setSavePopup(ActionEvent event) {

...
public class ProcesoMENGeneracionMensajesAction extends
	...
	public String setValidarInterfaz(){
		...
		String fechaFacturacion = f.getFechaFacturacion();
		String codigoPeriodo = f.getCodigoPeriodo();
		
		AjaxService ajaxService = (AjaxService) getBean("ajaxService");
		mensaje = ajaxService.getMessageRangoFechaActividad(codigoPeriodo, fechaFacturacion);  
		...
		Integer contador = service.getVerRegionesZonasAprobadasNoProc(criteria);
		
		if (contador.intValue() > 0) {
			mensaje = this.getResourceMessage("procesoMENGeneracionMensajesForm.existen.zonasAprobadas");
			return mensaje;
		}
		return mensaje;

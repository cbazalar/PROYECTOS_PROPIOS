...
@ManagedBean
@SessionScoped
public class MantenimientoCARNivelRiesgoSeccionSearchAction extends
	...
	protected BaseEditForm setObtenerRegistroAttributes() throws Exception {
		...
		for (Base nivelRiesgo : niveles) {
			if (nivelRiesgo.getDescripcion().equals(
					mantenimientoForm.getNivelRiesgo())) {
				mantenimientoForm.setOidNuevoNivelRiesgo(nivelRiesgo
						.getCodigo());
				break;
			}
		}
	...
	public void loadZonas(ValueChangeEvent val) {
		try {
			...
			if (!val.equals(null) && regiones.length > 0) {
				this.siccZonaList = Arrays.asList(aSvc
						.getZonasMultipleByPaisMarcaCanalRegion(
								this.mPantallaPrincipalBean.getCountryCode(),
								"T", "VD", regiones, "T"));
	...
	public void loadSecciones(ValueChangeEvent val) {
		try {
			...
			if (!val.equals(null) && form.getCodigoRegion().length() > 0) {
				this.siccSeccionList = Arrays.asList(aSvc
						.getSeccionesByPaisMarcaCanalRegionZona(
								this.mPantallaPrincipalBean.getCountryCode(),
								"T", "VD", form.getCodigoRegion(), val
	...
	/**
	 * @param carNivelRiesgoList
	 *            the carNivelRiesgoList to set
	 */

...
public class StringUtil {
			...
			if (today.get(Calendar.MONTH) <= birth.get(Calendar.MONTH)) {
				if (today.get(Calendar.MONTH) == birth.get(Calendar.MONTH)) {
					if (today.get(Calendar.DATE) > birth.get(Calendar.DATE)) {
						factor = -1; //Aun no celebra su cumpleańos
					}
				} else {
					factor = -1; //Aun no celebra su cumpleańos
				}
	 ...
	 * @return
	 */
    public static boolean validarTextoCaracteresNoValidos(String cadenaCaracteres, String texto)
	{
    	cadenaCaracteres = "__" + cadenaCaracteres;
    	
    	if(StringUtils.isBlank(texto))
    		return true;

    	if(StringUtils.isNotBlank(cadenaCaracteres))
    	{
        	for(int i=0; i<texto.length(); i++) {
        		String aux = "__" + String.valueOf((int)texto.charAt(i)) + "__";

        		if(cadenaCaracteres.indexOf(aux)>=0) {
        			return false;
        		}
        	}    		
    	}
	 ...
	 * @return
	 */
    public static boolean validarTextoCaracteresValidos(String cadenaCaracteres, String texto)
	{
    	cadenaCaracteres = "__" + cadenaCaracteres;
	...
	/**
	 * 
	 * @param validarCaracteres
	 * @return
	 */
    public static String obtenerListaCaracteres(String validarCaracteres) {
    	String cadenaCaracteres ="";
    	
    	String []listaCaracteres = StringUtils.split(validarCaracteres, "__");

    	for(int i=0; i<listaCaracteres.length; i++)
    	   cadenaCaracteres = cadenaCaracteres + " " + Character.toChars(Integer.parseInt(listaCaracteres[i]))[0];

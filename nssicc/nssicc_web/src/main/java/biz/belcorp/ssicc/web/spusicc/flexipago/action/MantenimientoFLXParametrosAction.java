...
public class MantenimientoFLXParametrosAction extends BaseMantenimientoSearchAbstractAction{
	...
	protected boolean setSaveAttributes() throws Exception {
		...
		ParametroForm p10[] = f.getParametrosGrupo10();
		...
		for (int i = 0; i < p10.length; i++) {
			ParametroFLX parametro = new ParametroFLX();
			BeanUtils.copyProperties(parametro, p10[i]);
			parametros.add(parametro);
		}
	...
	protected void setViewAtributes() throws Exception {
		...
		List parametroGrupo10 = service.getParametrosByGrupo("10");
		...
		ParametroForm p10[] = new ParametroForm[parametroGrupo10.size()];
		...
		try {
			...
			for (int i = 0; i < parametroGrupo10.size(); i++) {
				p10[i] = new ParametroForm();
				BeanUtils.copyProperties(p10[i], parametroGrupo10.get(i));
			}
			
			f.setParametrosGrupo10(p10);

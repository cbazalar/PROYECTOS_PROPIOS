...
public class MantenimientoPERPercepcionesOtrosCanalesSearchAction extends BaseMantenimientoSearchAbstractAction {
	...
	protected List setFindAttributes() throws Exception {
		...
		if(f.getFechaEmisionComprobantePercepcion() != null) {			
			f.setFechaEmisionComprobantePercepcionD(DateUtil.convertDateToString(f.getFechaEmisionComprobantePercepcion()));			
		}else{
			f.setFechaEmisionComprobantePercepcionD(null);
		}
		...
		criteria.put("fechaEmisionComprobantePercepcion", f.getFechaEmisionComprobantePercepcionD());
	...
	/**
	 * *biz.belcorp.ssicc.web.spusicc.action.MantenimientoPERPercepcionesOtrosCanalesSearchAction#enviarFormulario;
	 * @param f
	 * @return
	 */
	...
	public void setFind() {
		...
		if (StringUtils.isNotBlank(f.getCodigoConsultora())) {
			if (f.getCodigoConsultora().length() <= this.longitudCampoClientes)
				f.setCodigoConsultora(StringUtils.leftPad(f.getCodigoConsultora(), this.longitudCampoClientes,
						"0"));
			else {
				int tamanio = f.getCodigoConsultora().length() - this.longitudCampoClientes;
        ...
        String[] datosCliente = ajax.getConsultoraByCriteria(f.getCodigoPais(), f.getCodigoConsultora());
        if(datosCliente == null){
        	this.addError("Error ", "Consultora no existe");
        	return;        	
        }else{
        	f.setDescripcionConsultora(datosCliente[1].toString());
        	f.setApellidoPaterno(datosCliente[2].toString());
        	f.setApellidoMaterno(datosCliente[3].toString());
        	f.setNombres(datosCliente[4].toString());
        	return;
        }
	}	
	...
	public void cerosIzquierda() {
		...
		if (StringUtils.isNotBlank(f.getCodigoConsultora())) {
			if (f.getCodigoConsultora().length() <= this.longitudCampoClientes)
				f.setCodigoConsultora(StringUtils.leftPad(f.getCodigoConsultora(),longitudCampoClientes,
						"0"));
			else {
				int tamanio = f.getCodigoConsultora().length() - longitudCampoClientes;
				String codFinal = f.getCodigoConsultora().substring(tamanio);
				f.setCodigoConsultora(codFinal);
			}
		}

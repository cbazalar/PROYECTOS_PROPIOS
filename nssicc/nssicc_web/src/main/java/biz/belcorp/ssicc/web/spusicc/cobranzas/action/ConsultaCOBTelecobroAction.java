...
public class ConsultaCOBTelecobroAction extends BasePopupAbstractAction{
	...
	private List cabeceraMovimientoBancoList;
	private List detalleMovimientoBancoList;

	private List detalleMovimientoCuentaList;
...
//		Seteo periodo actual		
	...
	protected List setFindAttributes() throws Exception {
		...
		if(accion!=null){
			...
			if(accion.equals(this.ACCION_ULTIMOS_MOVIMIENTOS)){
				...
				String oidConsolidado = externalContext.getRequestParameterMap().get("parametro");
				String oidMovimiento = externalContext.getRequestParameterMap().get("parametro2");
				String codigoTipoMovimiento = externalContext.getRequestParameterMap().get("parametro3");
				
				Map criteria = new HashMap();				
				criteria.put("oidConsolidado", oidConsolidado);
				criteria.put("oidMovimiento", oidMovimiento);
				criteria.put("codigoTipoMovimiento", codigoTipoMovimiento);
				
				if(StringUtils.equals(codigoTipoMovimiento, "B")){

					this.setCabeceraMovimientoBancoList(service.getCabeceraMovimientoBanco(criteria));
					this.setDetalleMovimientoBancoList(service.getDetalleMovimientoBanco(criteria));
					this.setPopUp("popUpMovimientoBanco");
				}else{
					if(StringUtils.equals(codigoTipoMovimiento, "D")){
						this.setDetalleMovimientoCuentaList(service.getDetalleMovimientoCuenta(criteria));
						this.setPopUp("popUpMovimientoCuenta");
					}else{
						this.setDetalleMovimientoList(service.getDetalleMovimiento(criteria));
						this.setPopUp("popUpDetalle");
					}
				}
	...
	/**
	 * @return the cabeceraMovimientoBancoList
	 */
	public List getCabeceraMovimientoBancoList() {
		return cabeceraMovimientoBancoList;
	}

	/**
	 * @param cabeceraMovimientoBancoList the cabeceraMovimientoBancoList to set
	 */
	public void setCabeceraMovimientoBancoList(List cabeceraMovimientoBancoList) {
		this.cabeceraMovimientoBancoList = cabeceraMovimientoBancoList;
	}

	/**
	 * @return the detalleMovimientoBancoList
	 */
	public List getDetalleMovimientoBancoList() {
		return detalleMovimientoBancoList;
	}

	/**
	 * @param detalleMovimientoBancoList the detalleMovimientoBancoList to set
	 */
	public void setDetalleMovimientoBancoList(List detalleMovimientoBancoList) {
		this.detalleMovimientoBancoList = detalleMovimientoBancoList;
	}

	/**
	 * @return the detalleMovimientoCuentaList
	 */
	public List getDetalleMovimientoCuentaList() {
		return detalleMovimientoCuentaList;
	}

	/**
	 * @param detalleMovimientoCuentaList the detalleMovimientoCuentaList to set
	 */
	public void setDetalleMovimientoCuentaList(List detalleMovimientoCuentaList) {
		this.detalleMovimientoCuentaList = detalleMovimientoCuentaList;
	}

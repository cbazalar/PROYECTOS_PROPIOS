...
//		sesion.setAttribute("totalProcesosSICC", listaProcesos.size());
	...
	{		
		...
		boolean validarProcesoRecomendaciones = false;
		...
		if(this.esSeleccionable) {
			...
			if((f.getListaInterfacesSeleccionadas()!=null) && (f.getListaInterfacesSeleccionadas().length>0)) {
				for(int i=0; i<f.getListaInterfacesSeleccionadas().length; i++) {
					if("GEN-118".equals(f.getListaInterfacesSeleccionadas()[i])) {
						validarProcesoRecomendaciones = true;
						break;
					}
				}
			}
			...
			if(listaProcesosTemp.size()>0) {
				...
				f.setIndicadorEjecucionSICC(Constants.NO);
				params.put("indicadorEjecucionSICC", Constants.NO);
			}	
		...
		if(!validarProcesoRecomendaciones && Constants.NO.equals(f.getIndicadorEjecucionSICC())) {
			InterfazSiCCService interfazSiCCService = (InterfazSiCCService) getBean("sisicc.interfazSiCCService");
			...
			for(int i=0;i<oidZonas.length;i++){
				...
				if(contCierreZona == 0){
					...
					String mensaje = this.getResourceMessage("procesoGENProcesarCierreZonaForm.msg.validacionCierreZona",parametrosMsg);
	...
	protected Map<String, Object> executeProcessBeforeInterfaz(Map<String, Object> params) throws Exception {
		...
		if(Constants.SI.equals(indicadorEjecucionSICC)) {
			...
			service.executeProcesarCierreZona(params);
		}
		return params;
	}

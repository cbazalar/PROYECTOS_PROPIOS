...
import biz.belcorp.ssicc.web.spusicc.mae.form.MantenimientoMAEEntidadGenericaTipoVinculoForm;

@ManagedBean
@SessionScoped
@SuppressWarnings({"rawtypes", "unchecked"}) 
public class MantenimientoMAEEntidadGenericaTipoVinculoAction extends BaseMantenimientoSearchAbstractAction 
{
	/**
	 * 
	 */
	private static final long serialVersionUID = -5274908664318049061L;
	...
	protected boolean setSaveAttributes() throws Exception 
	{
		if (log.isDebugEnabled()) {
			log.debug("Entering 'setSaveAttributes' method - MantenimientoMAEEntidadGenericaTipoVinculoAction");
		}
		
		MantenimientoMAEEntidadGenericaTipoVinculoForm f = (MantenimientoMAEEntidadGenericaTipoVinculoForm) this.formMantenimiento;
		...
		BeanUtils.copyProperties(tipoVinculo, f);
		
        try {
            if (isNew) {
            	System.out.println("isnew");
            	if(!service.getExisteTipoVinculo(tipoVinculo)){            		
	    			service.insertTipoVinculo(tipoVinculo, usuario);
            	}else{
    			...
    			params.put("oidTipoVinc", f.getOidTipoVinc());
    			
    			TipoVinculo vinculo = service.getTipoVinculo(params);
    			
    			if(vinculo.getCodigo().equals(f.getCodigo())){
    				if(service.getExisteTipoVinculo(tipoVinculo)){	
    	    			service.updateTipoVinculo(tipoVinculo, usuario);
    	            }else{
                		this.addError("Error: ", this.getResourceMessage("mantenimientoMAEEntidadGenericaForm.error"));
        	            return false;
                	}
	...
	public void inicializaPantallaMantenimiento() 
	{
		try {
			if(log.isDebugEnabled()){
				log.debug("setAddAttributes - MantenimientoMAEEntidadGenericaTipoVinculoAction");
			}
			
			MantenimientoMAEEntidadGenericaService service = (MantenimientoMAEEntidadGenericaService)getBean("spusicc.mantenimientoMAEEntidadGenericaService");
			Usuario usuario = this.mPantallaPrincipalBean.getCurrentUser();
			MantenimientoMAEEntidadGenericaTipoVinculoForm f = new MantenimientoMAEEntidadGenericaTipoVinculoForm(); 
			...
			f.setDescripcion("");
			f.setRecomendado(Constants.ACTIVO);			
			//FIN RESET
			...
			{			
				...
				String id = bean.get("oidTipoVinc").toString();	
				
				if(StringUtils.isNotBlank(id)){
					
					Map params = new HashMap();
					params.put("oidTipoVinc", id);
					
					TipoVinculo tipoVinculo = service.getTipoVinculo(params);
					BeanUtils.copyProperties(f, tipoVinculo);
					f.setNombreEntidad(getNombreEntidad());
					f.setOidTipoVinc(id);

...
import biz.belcorp.ssicc.web.spusicc.cobranzas.form.ConsultaCOBEjecutivoForm;

@SessionScoped
@ManagedBean
public class ConsultaCOBEjecutivoAction extends BaseConsultaAbstractAction {
	...
	private LabelValue[] siccAccionesCobranzaListAux;
	...
	protected void setAceptarPopup(ActionEvent event, String accion) {
		...
		if (accion.equals(this.POPUP_HIPCLIENTE)) {
			...
			if (this.busquedaHIPClientePOPUPSearchAction.isSeleccionoRegistro()) {	
				...
				ConsultaCOBEjecutivoForm busquedaForm = (ConsultaCOBEjecutivoForm)this.formBusqueda;				
	...
	protected BaseSearchForm devuelveFormBusqueda() throws Exception {
		return new ConsultaCOBEjecutivoForm();
	}
	...
	protected void setViewAtributes() throws Exception {
		...
		ConsultaCOBEjecutivoForm f = (ConsultaCOBEjecutivoForm) this.formBusqueda;
		// Carga de los combos
		...
		loadRegionesPeriodo(f.getCodigoPeriodo());
		//Carga combo etapas
		loadEtapasCobrador();
	...
	public void limpiar(ConsultaCOBEjecutivoForm f){
		f.setCodigoCartera("");		
	 ...
	 */
	private void setForm(ConsultaCOBEjecutivoForm f, Map map, List listaTelefonos) {
		...
		f.setGerenteZonaTab(map.get("nombreGerenteZona").toString());
		f.setLiderSeccionTab(map.get("nombreLiderSeccion").toString());
	 ...
	 * @return
	 */  	 
	private Map getCriteriaSearch(ConsultaCOBEjecutivoForm f) {
	...
	public void search(ActionEvent event){
		...
		try {		
			...
			f.setCodigoConsultoraSearch(StringUtils.isBlank(f.getCodigoConsultoraSearch())?null:f.getCodigoConsultoraSearch());
			f.setNumeroDocumentoSearch(StringUtils.isBlank(f.getNumeroDocumentoSearch())?null:f.getNumeroDocumentoSearch());

			listaConsultoras = service.getCarteraEjecutivoByFilter(getCriteriaSearch(f));
	 ...
	 * @return
	 */
	private Map getCriteriaSave(ConsultaCOBEjecutivoForm f) {
		Map criteria = new HashMap();
	...
	public void loadEtapasCobrador() {		
		ConsultaCOBEjecutivoForm f = (ConsultaCOBEjecutivoForm)this.formBusqueda;
		if (StringUtils.isNotBlank(f.getCodigoCobrador())) {
			AjaxService ajax = (AjaxService) getBean("ajaxService");
			LabelValue[] lista = ajax.getEtapasDeudaByPaisSociedadCobrador(f.getCodigoCobrador());			
			if(lista!=null && lista.length>0) {
				LabelValue lv = lista[0];
				f.setCodigoEtapa(lv.getValue());
			}
			this.setSiccEtapasList(lista);	
		} else 
			this.setSiccEtapasList(null);
	}
	
	public void loadAccionesFiltro(ValueChangeEvent val) {
		if (log.isDebugEnabled()) {
			log.debug("loadAccionesFiltro");
		}
		ConsultaCOBEjecutivoForm f = (ConsultaCOBEjecutivoForm)this.formBusqueda;
	...
	public void loadAccionesFiltroAux(ValueChangeEvent val) {
		if (log.isDebugEnabled()) {
			log.debug("loadAccionesFiltroAux");
		}
		ConsultaCOBEjecutivoForm f = (ConsultaCOBEjecutivoForm)this.formBusqueda;
		String codigoEtapa = f.getCodigoEtapa();
		if (StringUtils.isNotBlank(codigoEtapa)) {
			AjaxService ajax = (AjaxService) getBean("ajaxService");
			LabelValue[] lvLista = null;
			LabelValue[] lv = new LabelValue[1];
    ...
    public void loadAcciones(ValueChangeEvent val) {
        ...
        if (StringUtils.isNotBlank(accion) && StringUtils.isNotBlank(f.getCodigoEtapaConsultora())) {
            AjaxService ajax = (AjaxService) getBean("ajaxService");
            this.siccAccionesCobranzaListAux = ajax.getAccionesCobranzaByPaisSociedadEtapaTipoAccion(f.getCodigoPais(),
                    "",
                    f.getCodigoEtapaConsultora(),
                    accion);

        } else {
            this.siccAccionesCobranzaListAux = null;
        }
    }
	 ...
	 * @return
	 */
	private Map getCriteriaActualizar(ConsultaCOBEjecutivoForm f) {
		Map criteria = new HashMap();
	...
	public void showPopupHistorialGestiones(ActionEvent event){
		 ...
		 */
		 ...
		 public void saveGestionNext(ActionEvent event) {
				
			if (log.isDebugEnabled()) {
				log.debug("Entering 'saveGestionNext' method");
			}
	...
	public void linkHiperconsulta(ActionEvent event) {
		try {
			...
			this.consultaHIPDatosClienteAction.setPaginaPadre("consultaCOBEjecutivoForm");
	...
	public void linkLlamadaEntrante(ActionEvent event) {
		try {
			ConsultaCOBEjecutivoForm f = (ConsultaCOBEjecutivoForm)this.formBusqueda;	
			this.consultaCOBEjecutivoLlamadaEntranteAction.setPaginaPadre("consultaCOBEjecutivoForm");
	...
	/**
	 * @return the siccAccionesCobranzaListAux
	 */
	public LabelValue[] getSiccAccionesCobranzaListAux() {
		return siccAccionesCobranzaListAux;
	}

	/**
	 * @param siccAccionesCobranzaListAux the siccAccionesCobranzaListAux to set
	 */
	public void setSiccAccionesCobranzaListAux(
			LabelValue[] siccAccionesCobranzaListAux) {
		this.siccAccionesCobranzaListAux = siccAccionesCobranzaListAux;
	}

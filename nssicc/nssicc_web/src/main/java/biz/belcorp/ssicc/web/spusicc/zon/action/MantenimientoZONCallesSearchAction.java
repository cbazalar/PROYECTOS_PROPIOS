...
public class MantenimientoZONCallesSearchAction extends BaseMantenimientoSearchAbstractAction {
	...
	{	
		...
		String id = ((BigDecimal)criteria.get("oidCalle")).toString();
		
		if(StringUtils.isNotBlank(id) || StringUtils.isNotEmpty(id))
		{
			criteria.put("oidCalle", id);
	...
	protected String devuelveMensajeKeyEliminarOK() {
		return "mantenimientoZONCallesForm.cabecera.delete";
	}
	...
	protected BaseEditForm setObtenerRegistroAttributes() throws Exception 
	{
		...
		if (!this.accion.equals(this.ACCION_NUEVO)) 
		{
			...
			Map registroSeleccionado = (HashMap) this.beanRegistroSeleccionado;
			String id = ((BigDecimal) registroSeleccionado.get("oidCalle"))
					.toString();

			if (id != null) {
				Map callesMap = service.getZonCalle(Long.parseLong(id));
	...
	public void loadNivel2(ValueChangeEvent val){
		...
		if(StringUtils.isNotEmpty(val.getNewValue().toString()) || StringUtils.isNotBlank(val.getNewValue().toString()))
		{
			...
			if(StringUtils.isNotBlank(codigoUnidadGeografica) || StringUtils.isNotEmpty(codigoUnidadGeografica)){
	...
	private void loadNivel(String codigoUnidadGeografica, int NumeroNivel){
		...
		case 2:
			...
			this.setListaNivel3(null);
			this.setListaNivel4(null);
			break;
		case 3:
	...
	protected String devuelveMensajeKeySaveOK() {		
		MantenimientoZONCallesForm f = (MantenimientoZONCallesForm) this.formMantenimiento;
		if (f.isNewRecord()) {				
			return "mantenimientoZONCallesForm.cabecera.insert";
		} else {
			return "mantenimientoZONCallesForm.cabecera.update";
		}
	}

...
@ManagedBean
@SessionScoped
public class ProcesoCOMComisionRecuperacionAction extends
	...
	private DataTableModel datacomGerenteRetiradas;

	private Object[] beanSeleccionadoComGerente;

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * biz.belcorp.ssicc.web.framework.base.action.BaseProcesoHiloAbstractAction
	 * #setViewAtributes()
	 */
	@Override
	protected void setViewAtributes() throws Exception {
		...
		this.comTipoCaluloList = tramoService.getTipoCalculoList(pais
				.getCodigo());
		AjaxService ajax = (AjaxService) getBean("ajaxService");
		this.siccComisionList = ajax
				.getComisionesByTipo(Constants.CODIGO_TIPO_COMISIONISTA_DEFAULT);
		...
		this.siccTipoComisionRecuperacionList = tramoService
				.getTiposComisionistas(pais.getCodigo());
	...
	public void buscarEjecutar(ActionEvent event) {
		if (log.isDebugEnabled()) {
		...
		try {
			...
			if (StringUtils.equals(codigoBaseComision,
					Constants.COM_CODIGO_BASE_COMISION_07)) {
				...
				if (StringUtils.equals(form.getTipoComision(),
						Constants.COM_COMISION_TIPO_COMISION_GERENTE_ZONA))
					criteria.put("codigoTipoCargo",
							Constants.COM_CODIGO_TIPO_CARGO_GZ);
				else
					criteria.put("codigoTipoCargo",
							Constants.COM_CODIGO_TIPO_CARGO_GR);
				...
				List listaGerenteRetiradas = mantenimientoCOMComisionService
						.getListaMostrarGerentesRetiradas(criteria);
				...
				if (comGerenteRetiradasList.size() == 0)
					throw new Exception(
							this.getResourceMessage("procesoCOMComisionRecuperacionForm.noExistenGerenteRetiradas"));
				form.setMostrarListaBaseComision07(true);
				this.datacomGerenteRetiradas = new DataTableModel(
						this.comGerenteRetiradasList);
			} else {
				String data = ajax.validaComisionCalculada(pais.getCodigo(),
						form.getCodigoPeriodo(), form.getCodigoComision());
				if (data.equals("S"))
					this.getRequestContext()
							.execute(
									"PF('confirmDialogExecute3_confirmationDialogConfirmar').show()");
				else
					this.getRequestContext()
							.execute(
									"PF('confirmDialogExecute_confirmationDialogConfirmar').show()");
			}
	...
	protected Map<String, Object> executeProcess(Map<String, Object> params)
		...
		if (comision.getCodigoBaseComision().equals(
				Constants.CODIGO_BASE_COMISION_07)) {
			List listaGerenteRetiradas = form.getListaGerenteRetiradas();
			for (int i = 0; i < listaGerenteRetiradas.size(); i++) {
				Map criteria = (Map) listaGerenteRetiradas.get(i);
		...
		if (comision.getCodigoBaseComision().equals(
			...
			log.debug("Los parametros del executeCalcularComisionVentaNetaEfectiva son: "
					+ params.toString());
			reporteService.executeCalcularComisionVentaNetaEfectiva(params);
		} else if ((comision.getCodigoBaseComision().equals(
				Constants.CODIGO_BASE_COMISION_01)
				|| comision.getCodigoBaseComision().equals(
						Constants.CODIGO_BASE_COMISION_07)
				|| comision.getCodigoBaseComision().equals(
						Constants.CODIGO_BASE_COMISION_02) || comision
				.getCodigoBaseComision().equals(
						Constants.CODIGO_BASE_COMISION_04))
				&& StringUtils.equals(parametro.getValorParametro(), "1")) {
			log.debug("Los parametros del executeProcess son: "
					+ params.toString());
			reporteService.executeComisionRecuperacion(params);
		} else if (comision.getCodigoBaseComision().equals(
				Constants.CODIGO_BASE_COMISION_03)) {
			log.debug("Los parametros del executeComisionesCalculadasPorRegion son: "
					+ params.toString());
			reporteService.executeComisionesCalculadasPorRegion(params);
		} else if (comision.getCodigoBaseComision().equals(
				Constants.CODIGO_BASE_COMISION_06)) {
			log.debug("Los parametros del executeComisionesVentaRetail son: "
					+ params.toString());
	 ...
	 * @see
	 * biz.belcorp.ssicc.web.framework.base.action.BaseProcesoAbstractAction
	 * #prepareParamsBeforeExecute()
	 */
	@Override
	protected Map<String, Object> prepareParamsBeforeExecute(Map params, BaseForm form)
			throws Exception {
		params = super.prepareParamsBeforeExecute(params, form);
		ProcesoCOMComisionRecuperacionForm f = (ProcesoCOMComisionRecuperacionForm) this.formProceso;
		String tipoComision = f.getTipoComision();

		params.put("tipoComision", tipoComision);
		params.put("codigoComision", f.getCodigoComision());
		String codTipoCalculo = f.getCodTipoCalculo();
	...
	protected List setFindAttributes() throws Exception {
		...
		for (int i = 0; i < resultado.size(); i++) {
			ProcesoBatchActu procesoBatchActu = (ProcesoBatchActu) resultado
					.get(i);
			...
			mapaNroLote.put("codigoSistema",
					procesoBatchActu.getCodigoSistema());
			mapaNroLote.put("idProcesoBatch",
					procesoBatchActu.getIdProcesoBatch());

			List listaNroLoteMultiHilo = service
					.getObtieneNroLoteProcesoBatchActu(mapaNroLote);
			for (int j = 0; j < listaNroLoteMultiHilo.size(); j++) {
				NroLoteMultiHilo registroNroLoteMultiHilo = (NroLoteMultiHilo) listaNroLoteMultiHilo
						.get(j);
				mapaNroLote.put("numeroLote",
						registroNroLoteMultiHilo.getNumeroLote());
				List listaHistorico = historicoService
						.getHistoricosLotesMultiHilo(mapaNroLote);
	...
	protected void valoresPorDefault() {
		...
		form.setAnioInicTramo(new SimpleDateFormat("yyyy").format(new Date(
	...
	public void obtenerCodigoBase(ValueChangeEvent val) {
		...
		if (StringUtils.isNotBlank(valor) || StringUtils.isNotEmpty(valor)) {
			ProcesoCOMComisionRecuperacionForm form = (ProcesoCOMComisionRecuperacionForm) this.formProceso;
			AjaxService ajaxService = (AjaxService) getBean("ajaxService");
			String codigoBaseComision = ajaxService
					.getCodigoBaseComision(valor);
	...
	/**
	 * Ejeucta Comision de Gerente Retiradas para Comision de Base 07
	 * 
	 * @param mapping
	 * @param form
	 * @param request
	 ...
	 * @throws Exception
	 */
	public void ejecutarGerente(ActionEvent event) {
		ProcesoCOMComisionRecuperacionForm f = (ProcesoCOMComisionRecuperacionForm) this.formProceso;
		List listaGerenteRetiradas = new ArrayList();
		for (int i = 0; i < beanSeleccionadoComGerente.length; i++) {
			listaGerenteRetiradas.add(beanSeleccionadoComGerente[i]);
		}
		f.setMostrarListaBaseComision07(false);
		f.setListaGerenteRetiradas(listaGerenteRetiradas);
		f.setCodigoComision(f.getCodigoComisionBase07());
		f.setCodigoPeriodo(f.getCodigoPeriodoBase07());
		f.setTipoComision(f.getTipoComisionBase07());
		//super.executeProceso();
		executeProceso();
		beanSeleccionadoComGerente = null;
	}

	public void abrirPopup(ActionEvent event) throws Exception {
		int tamanio = this.beanSeleccionadoComGerente.length;
		try {

			if (tamanio == 0)
				throw new Exception(
						this.getResourceMessage("procesoCOMComisionRecuperacionForm.listaGerenteRetiradas.noSelection.validation"));
			this.getRequestContext()
					.execute(
							"PF('confirmDialogExecute2_confirmationDialogConfirmar').show()");
		} catch (Exception e) {
			// TODO: handle exception
			addError("Mensaje", obtieneMensajeErrorException(e));
		}
	...
	public void abrirPopupEliminar(ActionEvent event) throws Exception {
		int tamanio = this.beanSeleccionadoComGerente.length;
		try {
			...
			this.getRequestContext()
					.execute(
							"PF('confirmDialogExecuteEliminar_confirmationDialogConfirmar').show()");
		} catch (Exception e) {
			// TODO: handle exception
			addError("Mensaje", obtieneMensajeErrorException(e));
		}
	...
	/**
	 * Elimina Gerente Retiradas de Comision de Base 07
	 * @param mapping
	 * @param form
	 * @param request
	 * @param response
	 ...
	 * @throws Exception
	 */
	public void eliminarGerente(ActionEvent event) throws Exception {
		ReporteService reporteService = (ReporteService) getBean("scsicc.reporteService");
		MantenimientoCOMComisionService mantenimientoCOMComisionService = (MantenimientoCOMComisionService) getBean("spusicc.mantenimientoCOMComisionService");
		ProcesoCOMComisionRecuperacionForm f = (ProcesoCOMComisionRecuperacionForm) this.formProceso;
		Pais pais = this.getmPantallaPrincipalBean().getCurrentCountry();
		List listaGerenteRetiradas = new ArrayList();
		try {
			Map criteria = (Map) beanSeleccionadoComGerente[0];
			String estado = (String) criteria.get("estado");
					
		    if (StringUtils.equals(estado, Constants.COM_GERENTE_RETIRADAS_ESTADO_PENDIENTE)) {
				throw new Exception(
						this.getResourceMessage("procesoCOMComisionRecuperacionForm.eliminarGerente.error.estadoPendiente"));
		    }
			...
			f.setCodigoRegionBase07(codigoRegion);
			f.setCodigoZonaBase07(codigoZona);
			f.setCodigoGerenteBase07(codigoCliente);
			f.setFechaRetiroBase07(fechaRegistro);
			
			/* Realizando la Eliminacion de Gerente Retiradas */
			Map params = new HashMap();
			params.put("codigoPeriodo", f.getCodigoPeriodoBase07());
			params.put("codigoRegion", f.getCodigoRegionBase07());
			params.put("codigoZona", f.getCodigoZonaBase07());
			params.put("codigoCliente", f.getCodigoGerenteBase07());
			params.put("codigoComision", f.getCodigoComisionBase07());
			params.put("tipoComision", f.getTipoComisionBase07());
			reporteService.executeEliminarComisionesGerenteRetiradas(params);	
			
			/* Volviendo a obtener la lista de Gerentes Retiradas */
			criteria = new HashMap();
			criteria.put("codigoPais", pais.getCodigo());
			criteria.put("codigoPeriodo", f.getCodigoPeriodoBase07());
			criteria.put("tipoComision", f.getTipoComisionBase07());
			criteria.put("codigoComision", f.getCodigoComisionBase07());
			if (StringUtils.equals( f.getTipoComisionBase07(), Constants.COM_COMISION_TIPO_COMISION_GERENTE_ZONA))
			...
			listaGerenteRetiradas = mantenimientoCOMComisionService.getListaMostrarGerentesRetiradas(criteria);
			f.setMostrarListaBaseComision07(false);
			this.comGerenteRetiradasList= listaGerenteRetiradas;
			beanSeleccionadoComGerente=null;
			addInfo("Mensaje", this.getResourceMessage("procesoCOMComisionRecuperacionForm.eliminarGerente.updated"));
	 ...
	 *            the new com tramo ejecutivas list
	 */
	 ...
	 * 
	 * @param comTipoCaluloList
	 *            the new com tipo calulo list
	 */
	 ...
	 * 
	 * @param siccTipoComisionRecuperacionList
	 *            the new sicc tipo comision recuperacion list
	 */
	 ...
	 * 
	 * @param comGerenteRetiradasList
	 *            the new com gerente retiradas list
	 */
	...
	public DataTableModel getDatacomGerenteRetiradas() {
		return datacomGerenteRetiradas;
	}

	public void setDatacomGerenteRetiradas(
			DataTableModel datacomGerenteRetiradas) {
		this.datacomGerenteRetiradas = datacomGerenteRetiradas;
	}

	public Object[] getBeanSeleccionadoComGerente() {
		return beanSeleccionadoComGerente;
	}

	public void setBeanSeleccionadoComGerente(
			Object[] beanSeleccionadoComGerente) {
		this.beanSeleccionadoComGerente = beanSeleccionadoComGerente;
	}

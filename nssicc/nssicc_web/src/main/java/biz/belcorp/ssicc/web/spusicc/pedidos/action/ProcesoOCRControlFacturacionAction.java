...
public class ProcesoOCRControlFacturacionAction extends
	 ...
	 * @throws Exception
	 */
	public void cerrarCampana(ActionEvent evt) throws Exception {
		...
		try {
			...
			this.find();
			this.addInfo("Información",
					this.getResourceMessage("proceso.concluido"));
		} catch (Exception e) {
	...
	private PedidoControlFacturacion getControlBean(
		...
		controlFacturacion.setCodigoPeriodo(controlFacturacion
				.getCodigoPeriodo());
	...
	public String setValidarConfirmar(String accion) {
		...
		if (accion.equals("LOTE")) {
			...
			String id = sistemabusqueda.getCodigoPais() + "-"
					+ sistemabusqueda.getCodigoPeriodo();
			LabelValue retorno = ajax.getNumeroPedidosByLote(id);
			String retornoMensaje = this
					.getResourceMessage("confirm.actualiza.lote.message");
			if (retorno != null) {
				if (retorno.getValue().equals("0")) {
					retornoMensaje = this
							.getResourceMessage("mensaje.confirm.cantidad.cero")
							+ " "
							+ retorno.getLabel()
							+ " "
	...
	protected boolean setDeleteAttributes() throws Exception {
		...
		String mensaje = this
				.getResourceMessage("actualiza.contrario.indicadores");
		this.addInfo("Infomación:", mensaje);
		return true;
	}
	...
	protected boolean setSaveAttributes() throws Exception {
		...
		Usuario usuario = this.mPantallaPrincipalBean.getCurrentUser();
		controlFacturacionForm.setFechaProceso(DateUtil.convertDateToString(controlFacturacionForm.getFechaProcesoD()));
	...
	protected BaseEditForm setObtenerRegistroAttributes() throws Exception {
		...
		if (!this.accion.equals(this.ACCION_NUEVO)) {
			...
			criteriaAux.put("codigoPeriodo", periodoSigiente);
			criteriaAux.put("numCampanhas", "-1");
			String periodoAnterior = service.getPedidosNSiguienteCampanha(criteriaAux);
			
			criteriaAux.put("codigoPais", pais.getCodigo());
			criteriaAux.put("codigoPeriodo", periodoAnterior);
			List listFacturacion = service.getControlFacturacionByCriteria(criteriaAux);
			
			if(listFacturacion.size()==0) {
				controlFacturacionForm.setMontoMinimoDeuda(new BigDecimal(0));
				controlFacturacionForm.setUnidadesMaximo(0);
			} else {
				PedidoControlFacturacion pedidoControl = (PedidoControlFacturacion)listFacturacion.get(0);
				controlFacturacionForm.setMontoMinimoDeuda(pedidoControl.getMontoMinimoDeuda());
				controlFacturacionForm.setUnidadesMaximo(pedidoControl.getUnidadesMaximo());
			}	

...
//		request.getSession(true).removeAttribute("listaFleteDetalle");
	}
	...
	public void cargarData() throws Exception{
		...
		BigDecimal idBigDecimal = new BigDecimal(seleccionado.get("oidFlete").toString());
		String id = idBigDecimal.toString(); 
		Flete flete = servicePed.getFlete(id);
		...
		{
			this.stoTipoClasificacionList = ajax.getTipoClasificacionByOidSubTipoCliente(flete.getOidSubTipoCliente());
		}		
		if(!StringUtils.isBlank(flete.getOidClasificacion()))
		{
			this.stoClasificacionList = ajax.getClasificacionByOidTipoClasificacion(flete.getOidTipoClasificacion());
		}
		if(!StringUtils.isBlank(flete.getCodigoZona()))
		{
		 ...
		 BigDecimal bd  =  new BigDecimal(mForm.getMontoFijo());
		 int valor = bd.intValue();

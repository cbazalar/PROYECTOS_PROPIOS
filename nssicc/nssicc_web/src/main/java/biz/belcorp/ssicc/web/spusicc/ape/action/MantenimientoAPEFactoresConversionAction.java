...
public class MantenimientoAPEFactoresConversionAction extends BaseMantenimientoSearchAbstractAction{
	...
	public void loadUnidadMedidas(ValueChangeEvent event) throws Exception{
	    	
			Pais pais = this.mPantallaPrincipalBean.getCurrentCountry();
			String codigoMagnitud = (String)event.getNewValue();
			AjaxService ajax = (AjaxService) getBean("ajaxService");
			this.listaUnidades= ajax.getUnidadesMedidaListar( pais.getCodigo(), codigoMagnitud);
		}
	public void validarFactor() throws Exception
	{
		MantenimientoAPEFactoresConversionForm f = (MantenimientoAPEFactoresConversionForm) this.formMantenimiento;
		String factor = f.getFactor();
		String[] codigoCompuesto;
		codigoCompuesto= factor.split(".");
		int numeroEntero = 0;
		int numeroDecimal = 0;
		try
		{
		if(codigoCompuesto.length > 0)
		{	
			if(codigoCompuesto[0]!=null && codigoCompuesto[0].length()>0)
			{
				numeroEntero = Integer.parseInt(codigoCompuesto[0]);
				if(numeroEntero>9999)
				{
					addWarn("Mensaje: ", getResourceMessage("mantenimientoAPEFactoresConversionForm.msg.factor"));
					f.setFactor("");
				}
			}
			
			if(codigoCompuesto[1]!=null && codigoCompuesto[1].length()>0){
				numeroDecimal = Integer.parseInt(codigoCompuesto[1]);
				if(numeroDecimal>999)
				{	addWarn("Mensaje: ", getResourceMessage("mantenimientoAPEFactoresConversionForm.msg.decimal"));
					f.setFactor("");
				}	
			}
		}
			...
			{
				numeroEntero = Integer.parseInt(factor);
			}
		}
		catch (Exception e)
		{
